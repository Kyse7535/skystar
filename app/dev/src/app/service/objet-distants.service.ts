...
export class ObjetDistantService {
  ...
  private apiUrl = `http://localhost:8050/api/objet_distants`;  // URL to web api

  constructor(
    private http: HttpClient) { }

  getObjetDistants(params?: HttpParams): Observable<ObjetApi> {
    return this.http.get<ObjetApi>(this.apiUrl, { params })
      .pipe(
  ...
  getObjetDistant(id: number): Observable<ObjetDistant> {
    return this.http.get<ObjetDistant>(this.apiUrl+"/"+id)
      .pipe(
